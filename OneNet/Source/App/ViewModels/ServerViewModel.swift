...
class ServerViewModel : ObservableObject {
    ...
    private func getAuthenticator(url managementServerUrl: String) async -> OneNetSDKAuth? {
        let configPath = self.configurationFilePath
        let detachedTask = Task.detached(priority: .background) {
            var error: NSError?
            var errorMessage : String?
            var authenticator : OneNetSDKAuth?
            authenticator = OneNetSDKNewAuth(configPath, managementServerUrl, &error)
