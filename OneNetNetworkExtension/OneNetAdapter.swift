...
public class OneNetAdapter {
    ...
    public let client : OneNetSDKClient
    ...
    init(with tunnelManager: PacketTunnelProviderSettingsManager) {
        ...
        self.client = OneNetSDKNewClient(Preferences.configFile(), Preferences.stateFile(), Device.getName(), Device.getOsVersion(), Device.getOsName(), self.networkChangeListener, self.dnsManager)!
    }
    ...
    public func start(completionHandler: @escaping (Error?) -> Void) {
        ...
        DispatchQueue.global().async {
            do {
                ...
                completionHandler(NSError(domain: "network.l1fe.onenetwork.NetworkExtension", code: 1001, userInfo: [NSLocalizedDescriptionKey: "Onenet client startup failed."]))
                self.stop()
            }
        }
    }
