...
class PacketTunnelProvider: NEPacketTunnelProvider {
    ...
    private lazy var adapter: OneNetAdapter = {
        return OneNetAdapter(with: self.tunnelManager)
    }()
    ...
    override func startTunnel(options: [String : NSObject]?, completionHandler: @escaping (Error?) -> Void) {
        ...
        if adapter.needsLogin() {
            DispatchQueue.main.asyncAfter(deadline: .now() + 2) {
                let error = NSError(
                    domain: "network.l1fe.onenetwork.NetworkExtension",
                    code: 1001,
...
func initializeLogging(loglevel: String) {
    ...
    if let logPath = logURL?.path {
        success = OneNetSDKInitializeLog(loglevel, logPath, &error)
    }
