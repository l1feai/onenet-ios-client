...
class Preferences {
    static func newPreferences() -> OneNetSDKPreferences {
        guard let prefs = OneNetSDKNewPreferences(configFile(), stateFile()) else {
            preconditionFailure("Failed to create OneNetSDKPreferences")
        }
        return prefs
    }
